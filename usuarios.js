    ...
    try {
        ...
        for (let i = 0; i < data.rows.length; i++) {
            let usuarios = {
                ...
                ciudad_us: data.rows[i][6],
                contrasenna_us: data.rows[i][7],
...
router.post('/crear', async function(request, response) {
    ...
    try {
      ...
      const insercion = await connection.execute(
        "INSERT INTO usuarios VALUES (seq_usuario.nextval, '" + request.body.correo_us + "', '" + request.body.nombre_us + "', '" + request.body.primer_apellido_us + "', '" + request.body.segundo_apellido_us + "', '" + request.body.celular_us + "', '" + request.body.ciudad_us + "', '" + request.body.contrasenna_us + "')"
      );
...
router.put('/:id/', async function (request, response) {
  ...
  try {
    ...
    const ciudad_us = request.body.ciudad_us;
    ...
    const result = await connection.execute(
      'UPDATE usuarios SET nombre_us = :nombre, correo_us = :correo, primer_apellido_us = :primer_apellido, segundo_apellido_us = :segundo_apellido, celular_us = :celular, ciudad_us = :ciudad, contrasenna_us = :contraseña WHERE id_us = :id',
      { 
        id: id_us,
        correo: correo_us,
        nombre: nombre_us,
